-- can_rejoin RPC v2: has_record 필드 추가
-- 재가입 유저 감지용 (withdrawn_users에 기록이 있는지 여부)
-- 클라이언트에서 RLS 때문에 직접 조회 불가하므로 RPC 통해 확인
...
BEGIN
    ...
    -- 레코드 없음 → 가입 가능, 탈퇴 이력 없음
    IF NOT FOUND THEN
        RETURN QUERY SELECT true, NULL::TEXT, NULL::TIMESTAMPTZ, false;
        RETURN;
    END IF;
    ...
    IF latest_record.withdrawal_type = 'error_resolution' THEN
        RETURN QUERY SELECT true, NULL::TEXT, NULL::TIMESTAMPTZ, true;
        RETURN;
    END IF;

    -- banned = 영구 차단
    IF latest_record.withdrawal_type = 'banned' THEN
        RETURN QUERY SELECT false, '영구 차단된 계정입니다.'::TEXT, NULL::TIMESTAMPTZ, true;
        RETURN;
    END IF;
    ...
    IF latest_record.withdrawal_type = 'abuse_concern' THEN
        IF latest_record.rejoin_allowed_at IS NOT NULL AND latest_record.rejoin_allowed_at > NOW() THEN
            RETURN QUERY SELECT false, '재가입 대기 기간입니다.'::TEXT, latest_record.rejoin_allowed_at, true;
            RETURN;
        END IF;
        -- 대기 기간 만료 → 가입 가능
        RETURN QUERY SELECT true, NULL::TEXT, NULL::TIMESTAMPTZ, true;
    ...
    IF check_ip IS NOT NULL THEN
        ...
        IF FOUND THEN
            RETURN QUERY SELECT false, '차단된 IP입니다.'::TEXT, NULL::TIMESTAMPTZ, true;
            RETURN;
        END IF;
    END IF;
