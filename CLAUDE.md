...
## 🔁 인계 누락 방지 (필수 — 세션 간 누락 0 목표)

> 배경: 세션 간 인계에서 "작업했는데 RELAY에 안 적혀 다음 세션이 놓치는" 누락이 반복됨.
> 원인: RELAY 비대화 / 메모리 인덱스만 읽힘 / commit↔RELAY 비동기. → 아래 규칙으로 구조적 차단.

### 세션 시작 시
- `RELAY.md` 상단 3섹션(🔴NEXT / ⚠️미검증 / ⏳외부대기)**만** 읽으면 이어서 할 일·누락이 0이어야 한다. 이 30~50줄이 단일 진실의 인덱스.

### 작업 완료 즉시 (누적 금지)
- 완료 항목은 RELAY 보드에서 **빼고** `RELAY-LOG.md` 맨 위에 `YYYY-MM-DD 커밋ID 한줄` 추가.
- RELAY.md에 완료 항목을 쌓지 않는다. 보드는 항상 50줄 이내.

### 세션 끝마다 (강제)
- **`/handover` 스킬 실행**. git log(마지막 인계 이후) ↔ RELAY 보드 자동 대조 → 보드에 없는 커밋·미반영 미검증을 검출해 상단 3섹션 갱신. 사람 기억이 아니라 git이 진실.
- 커밋 메시지에 항상 `검증: Lx` + 남은 검증을 적는다(기존 검증 워크플로와 동일). 보드는 그 인덱스일 뿐.

### 메모리 갱신
- 영구 교훈은 `MEMORY.md` 인덱스 **+ 본문 .md 둘 다** 갱신. 인덱스 한 줄만 갱신 금지(다음 세션이 본문 안 봐서 디테일 누락).
...
- 모드별 테마 색상 적용
- 타입 안전성 확보
- **웹↔모바일 패리티 (필수)**: UI/기능/로직을 한쪽(`src/` 또는 `mobile/`)에서 바꾸면 **반드시 `docs/PARITY.md`의 대응 파일을 열어 같은 변경/같은 버그가 있는지 확인 후 양쪽 동시 처리.** 한쪽만 고치고 "완료" 금지. 보고 시 "웹 Lx / 모바일 Lx(또는 해당없음)" 둘 다 명시. (데스크탑웹=모바일웹=`src/` 같은 코드, 앱만 `mobile/` 별도라 누락 잦음 — 반복 지적된 사항)
