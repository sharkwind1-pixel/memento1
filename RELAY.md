...
2. **chat-prompts.ts 추모 프롬프트 추가**:
```
## 실용 질문 대응
사용자가 장례/화장/수습/비용/장소 등 실질적 정보를 물으면:
- 감정 위로만 하고 넘기지 않는다
- 캐릭터 톤 유지하면서도 구체적 정보를 전달한다
- "나도 잘 모르지만, 이런 곳들이 있다고 들었어..." 식으로 자연스럽게
- 검색 결과가 주입되면 그 정보를 바탕으로 답변
```
3. **isCareRelatedQuery에 추모 실용 키워드 추가**: 장례, 화장, 수습, 장묘, 납골, 추모공원, 비용
