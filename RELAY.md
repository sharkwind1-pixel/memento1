...
### 결제 — KCP `[3014] 미등록 또는 사용불가 사이트 코드`

2026-04-16 새벽 시도 시 3014 에러 지속. 코드/포트원 콘솔 설정은 전부 정상 확인됨.

**코드 측 상태 (모두 OK)**:
- 포트원 콘솔 실연동 채널(`mementoani-real`): MID `A52LD`, 채널키 `channel-key-6695f681-...`, 사이트키 `4wtvaFYQknd3bJLVxa03mHO__`, 정기그룹ID `A52LD1000913` 전부 정확
- 프론트 페이로드: `pg: "kcp_billing.A52LD"` + `channelKey` + `customer_uid` 모두 정상 전송 확인됨 (PR #16)
- KCP 상점관리자(admin8.kcp.co.kr) 로그인 가능, 사이트 상태 "정상"

**원인 후보 (외부 확인 필요)**:
1. KCP 측 사이트에 **정기결제(billing) 모듈 계약/활성화**가 안 됐을 가능성 (가장 유력)
2. KB국민카드 심사 대기로 정기결제 전체가 보류됐을 가능성
3. 정기그룹ID `A52LD1000913` KCP 서버 측 등록 지연

**내일(04-17 금) 09시 이후 할 일** (이 순서 그대로):
1. **KCP 가맹점 운영상담 1544-8660** 전화
   ```
   "상점ID A52LD 메멘토애니입니다. 포트원 경유 정기결제 시도 시
   [3014] 미등록 사이트 에러가 납니다.
   - pg=kcp_billing.A52LD 명시했는데도 동일
   - 정기그룹ID A52LD1000913 활성화 상태
   - KB국민카드 심사 대기로 정기결제 전체 보류 여부
   이 세 가지 확인 부탁드립니다."
   ```
2. **포트원 콘솔 우측 하단 채팅** 또는 `cs@portone.io` 병행 문의
   ```
   상점: store-db90fdc1-08f5-4c24-855f-228e1abe1b32
   채널: mementoani-real
   "KCP 측에 A52LD + A52LD1000913 정기결제 모듈 활성화 여부 확인 요청"
   ```
3. 해결되면 시크릿 모드 → 베이직 9,900원 → KCP 상점관리자에서 즉시 취소

**디버그 참고**:
- `payments.metadata.debug_*` 필드가 기록되도록 양쪽 엔드포인트(complete, subscribe/complete) 모두에 진단 코드 있음 (PR #14, #15)
- 다음 실패 시 Supabase MCP로 `SELECT id, metadata FROM payments ORDER BY created_at DESC LIMIT 3;` 조회 가능
