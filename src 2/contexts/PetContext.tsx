...
interface PetContextType {
    ...
    updatePhoto: (petId: string, photoId: string, data: Partial<PetPhoto>) => void;
...
export function PetProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        try {
            ...
            if (savedPets) {
                ...
                if (savedSelectedId && parsed.some((p: Pet) => p.id === savedSelectedId)) {
    ...
    useEffect(() => {
        if (!isLoading && selectedPetId) {
            try {
                ...
                console.error("Failed to save selected pet to localStorage:", error);
            }
        }
    ...
    // Pet CRUD
    const addPet = useCallback((petData: Omit<Pet, "id" | "createdAt" | "photos">): string => {
    ...
    // Photo CRUD
    const addPhotos = useCallback((petId: string, photos: Omit<PetPhoto, "id">[]) => {
    ...
    const updatePhoto = useCallback((petId: string, photoId: string, data: Partial<PetPhoto>) => {
        setPets((prev) =>
            prev.map((pet) =>
                pet.id === petId
                    ? {
