...
## 2026-06-14 신규 닉네임 모달 부활 + 튜토리얼 투어 3세그먼트 개편
...
- ✅③첫화면 세그먼트 카드(웹, 커밋 5ae96a5): `SegmentActionCards.tsx` 홈 히어로 직후. onboardingData.userType 분기 — planning=반려사주(openSaju)+입양정보 / current=케어리마인더+오늘기록+AI펫톡 / memorial=추억돌아보기+마음나누기(amber). **라이브 L5**(current 계정 "오늘 우리 아이와" 3카드 렌더 확인). 이로써 9번 사주 진입점 머스트 해소(웹). 유형 미상이면 미표시.
- ✅②/③ 모바일 패리티(커밋 05056fb·0727bc6): 모바일 SajuModal(RN, /api/saju 호출, 마스킹 날짜입력) + SegmentActionCards(onboarding_data.userType 분기, planning=사주모달) 홈 연결. 9번 SHIP WITH FIXES→소스(onboarding_data.userType, 웹과 동일)·문구 통일 반영. app/ tsc 0 + esbuild OK. **L5(실기기)는 EAS 빌드 필요.**
- 잔여(다음 순서): ④보안 심화(RLS 정책 전수/Storage 버킷 권한) → ⑤신규가입 E2E(닉네임+투어+사주 L5) + EAS빌드(앱 닉네임 실기기).
...
- **9번 적대검증(2026-06-12, prod DB 실측) → SHIP OK**: RLS forge 실삽입 차단·self 23514·mutual 양탭 합성그래프·dedup unique·CASCADE·페이지바운드 7체크 전부 통과, 필수수정 0(L4). **비차단 후순위 2건**: ①unfollow 시 `neighbor_follow:follower:target` notification 행 삭제(현재 dedup_key 영구잔존 → 정당한 재팔로우 알림 1회성) ②settings 행 부재=공개는 이웃 게이트(route.ts:42)+minihompy GET 2곳 공동 결정 — "부재=비공개" 전환 시 동시수정 필요 주석화.
...
- **앱**: 펫홈 화면에 동일 섹션 칩 → record 서브탭 직행(신규 openTab 파라미터, openVideo 패턴). 웹=임베드/앱=네비(v1 미세차이 — 앱 record에 동일 탭 기존재).
- **웹 L5 PASS**(prod): 6섹션 전부 검증 — 홈/사진첩(그리드 1장)/다이어리(스크린샷), AI영상/방명록/케어(DOM 실측 — 헤더 h3·"방명록 (0)"·"케어 리마인더"). 헤더 카운트 "사진 1·방명록 0·방문 0" 정확.
- **⚠️디버깅 교훈**: 검증 중 CDP 스크린샷이 연속 timeout("renderer frozen") → 내 코드 렌더루프 의심했으나 **JS 평가(Runtime.evaluate)로 렌더러 생존+섹션 정상 마운트 입증** → 브라우저 확장의 캡처 채널만 죽은 것(탭 수명 갈수록 짧아지는 퇴화 패턴). **"frozen" 판정 전에 다른 CDP 채널(JS)로 생사 교차확인할 것.**
- 모바일 L2(tsc)·EAS 후 실기. 다음 = **이웃**(사용자 합의 순서).
...
- **모바일**: 이미 있던 빈 상태 "상점 둘러보기" CTA를 "펫홈을 꾸며보세요"로 문구 정렬 + 동일 `PethomeStartGuideModal` 추가(설명 단계 패리티). 모바일이 오히려 빈 상태가 먼저 있었음(웹이 비어있던 쪽).
- **미세 차이(기재)**: 빈 상태 판정 — 웹=`placed0 && 인사말없음`, 모바일=`placed0 && 보유꼬미0`(기존 로직 유지). 새 유저 케이스는 양쪽 동일, 엣지(보유했으나 미배치)만 상이.
- 검증: 웹 L2(tsc+build exit0) / 모바일 L2(tsc exit0). 배포 후 L5(웹: 새 계정 펫홈→오버레이/가이드/상점) · EAS 후(앱) 시각확인 잔여.
...
- **9번 적대 검산이 치명오류 발견**(`fd20c81`): 창업중심대학 SOM **9.5억→약9,500만원**(800×118,800=95,040,000, 자릿수 10배 오류=과거 탈락사유 재발). SOM 근거 1년차→2년차(월매출 표와 일치). TAM 1조8,413억 정밀화. BEP "12개월(800명)"→"약12개월·1년차 총매출 도달"(800명은 2년차라 모순) 양 파일.
- 잔존 옛값(7,900/142,200/17.8x/6.3억/월5회) grep 0건. 9번 최종 SHIP OK.
...
- **Fix B 펫톡**: (1)`howWeMetLabel` 신설 — enum(보호소 등)을 "보호소에서 입양" 서사형으로(빈약한 코드단어 직주입 개선), 3호출부 적용. (2)**본인기록 안 잘리게** — `timelineToContext` 30개무제한 → 최근10개+본문80자 캡, photo 12개 캡 + chat-pipeline 우선순위 timeline/photo를 최하위(2/1·3/3)→mid(5/4)로. 예산 초과 시 본인 일기/사진이 먼저 잘리던 역설 해소(캡+상향 짝이라 다른 컨텍스트 안 굶김).
검증: tsc clean + vitest 129 + next build ✓(L2) + **9번 적대검증 회귀 0건 SHIP가**(소유권/게이트범위/인젝션 false-positive無/캡·우선순위 정렬 전수). refusal 정규식 `unable to` 단독오탐 여지 1줄 좁힘. ⏳ **Fix C 모바일 영상 템플릿 패리티(13<20 + 커스텀 입력 부재) 별도 진행 예정.** ⚠️ 잔여: petId 미전달 시 추모게이트/종주입 무력화(방어우회 여지, 정상회귀 아님).
...
## [2026-05-26 추가] 농식품 창업콘테스트 사업계획서 작성·제출 (비코드)

재도전성공패키지(추경) 탈락 평가의견 수령(총점 64.9/100, 약점: 성장전략 65% · 팀 0점=분량초과 사고). 그 분석으로 **농식품 창업콘테스트(2026, 한국농업기술진흥원, 마감 6/3 18시) 양식1~6 전수 작성·제출 완료**. 신청분야 "반려동물", 팀명 "꼼지네형", *청년(1989생)+기창업(2026.3.16)* 이중 우대 트랙(공고문상 "예비창업자 또는 창업 1년 이내" 1팀 반드시 시상 + 청년프론티어상 후보). 양식1 13페이지(15p 이내, **분량초과 사고 재발 0**). 재도전 원본 자산 재활용 + 평가위원 약점 보강(분량/성장전략/치유근거) + 농식품 신규 섹션(농산업 후방분야 활성화/농가 연계/지역상생/AI·AX). **SOM 2단계** 신규 산식(1년차 0.5억 거점 + 3년차 6억 — 재도전 "6.3억=800명×검산오류" 수정). 학술 인용 9번 팩트체커로 안전버전(KB54.7%/83.2% 검증분만, Adams·Bonas/AVMA 2022/38.2%5%미만 등 미검증분 삭제). 가격 정책 코드 진실(`constants.ts` 단일프리미엄 월9,900/연88,800·AI영상 4,900/19,900/34,900)과 일치. 사업자정보: 687-08-03135, 도봉세무서, 서울 강북. 폐업이력: PR-B(스마트스토어 의류, 2017.03~2024.01, 가족자금사정). **수정 패치 5회** 반복 후 사용자 직접 제출(가격모순/모바일출시6월/협약기간잔재/이메일오타/구분칸 등). 다음: 결과 대기(서류 6.8~14→66팀, 발표 6월중순→22팀, 본선 7월초→11팀, 결선 9월초·전문가80+모의투자20·1팀당 모의 5억). 후속: 동일 자료 변형으로 서울 유니콘 챌린지(6/30) 신청 가능. 농식품 콘테스트 수상 시 **농식품 벤처육성 지원사업 가산점**(공고 명시).

## [2026-05-20 추가] 채팅 안 보임 회귀 + PKCE expo-crypto + 영구 소실 차단

(1) **0599b7e** 메모리얼 펫 데이터 앵커 saveMessage 에러 삼킴 *삼중결함* 수정 — memory.ts:248 null 반환·chat-pipeline.ts:1215 fire-and-forget·useAIChat.ts:388 catch{} 무시 → 3회 재시도 + localStorage/AsyncStorage 백업으로 영구 소실 차단(웹+앱). (2) **7201f85** 펫 사진 히어로(929b5ea) 이식 후 추모카드 ~325px가 KeyboardAvoidingView 컬럼 높이 먹어 키보드 시 채팅 FlatList 0으로 눌림(=채팅 안 보임 회귀) → 히어로+AI고지 배너를 FlatList ListHeaderComponent로 이동, 메시지 영역 풀 높이 복구. (3) **53fef38** PKCE OAuth 검증자가 Expo Go에서 `crypto.getRandomValues 미지원` 경고 + Math.random fallback 도달(보안 약화) → `expo-crypto` 설치 + `Crypto.getRandomBytes` 1순위로 CSPRNG 보장(Expo Go·EAS 둘 다). 모두 L2(typecheck/build·9번 통과). Expo Go 실기·EAS 빌드 후 베타 미검증.

---

## [2026-05-19 추가] 추모 세션종료 넛지 너무 일찍 → 30→60

`chat-pipeline.ts:887` 과사용 넛지("대화가 길어졌네요")가 추모모드 chatHistory 30메시지(~15왕복)에서 떠 위로 흐름 한창에 끊겼음. 사용자 결정으로 임계 30→60(~30왕복), 반복 주기 %10→%20 상향. 한 줄. 웹 typecheck 0 = L2.

---

## [2026-05-19 추가] 추천멘트 "내 목소리 들려?"/"잘 자" 하드코딩 제거

사용자 격분 "추천멘트에 내목소리 들려 같은 소리 왜 나오냐". 추측 안 하고 코드 추적 → **GPT 생성이 아니라 `chat-pipeline.ts:980` memorialFallbacks 배열에 `"내 목소리 들려?"` + `"잘 자"`(979)가 문자 그대로 하드코딩**돼 있었음. 화이트리스트(950)도 `들려|소리` 허용해 GPT 것도 통과. 3계층 차단: ① `chat-helpers.ts MEMORIAL_SUGGESTION_BLOCKLIST`에 목소리/숨소리/들려/들린/들렸/말투/잘 자/또 올게/잘 있어/좋은 하루 추가 ② memorialFallbacks에서 "내 목소리 들려?"·"잘 자" 제거(→"오늘 네 생각 났어" 등으로 교체) ③ GPT 파싱 단계에 VOICE_BLACKLIST(목소리|숨소리|들려|들린|들렸|말투) + CLOSING_BLACKLIST(^잘 자$|^안녕$|또 올게|잘 있어|좋은 하루) 하드 필터 추가(모드 무관). route.ts offTopicSuggestions는 깨끗(검증). 웹 typecheck 0, 빌드 진행. 교훈: "AI가 자꾸 X 한다" → 프롬프트 의심 전에 하드코딩 fallback/배열부터 grep. 프롬프트 규칙은 fallback 배열을 못 막는다.
...
## [2026-05-19 추가] 프롬프트 수술 — "..." 근절 + 목소리금지 + 추천멘트

"..." 가 안 빠진 진짜 원인: 추모 프롬프트가 **명시적으로** 말줄임표 권장(`권장: "...", ...` / `쉼표/말줄임표 자주`)하고 좋음 예시 다수가 "..."로 시작. 1차 때 few-shot 안 건드린 게 미스. 수정(chat-prompts.ts, 웹+앱 공통 단일소스): (1) 추모 `권장 "..."` 삭제+말줄임표 금지 규칙, 좋음 예시 전수 "..." 제거(433/434/437/441/447/448/481/484/490/424/426), 일상 few-shot "..."(162/166/170/132/192) 제거. (2) "목소리/소리 지금 듣는다" 금지 규칙 신설 — line 436 "네 목소리에서 피곤한 게 느껴져"(좋음예시였음)가 사용자 "텍스트인데 목소리 어떻게 듣냐" 격분 원인. 과거 살아있을 때 소리기억은 허용, 현재 텍스트에 소리반응 금지. 일상에도 대칭 규칙. (3) SUGGESTIONS: 인사/작별("잘 자","잘 지내","안녕","또 올게") 금지 — 후속질문은 대화 잇는 한 마디여야. 일상+추모 양쪽. 웹+모바일 typecheck 0, 웹 build exit 0 = L2. 실대화 체감 미검증(프롬프트라 실제 GPT 응답 봐야) → RELAY ⚠️. "여전히 웹 ux 다름"은 사용자가 구체 확인 보류(다음 지시 대기). 교훈: 프롬프트 톤 버그는 규칙 한 줄로 안 됨 — few-shot 예시가 진짜 교사. 예시까지 전수 수술.
...
**AI펫톡 웹1:1 재정렬** (사용자 원칙: 임의 재디자인 금지, 웹 그대로 이식. 대조 에이전트 1:1 대조표 → 9번 9.4/10):
- `203bf3a` 메시지 버블(펫버블 모드분기/유저그라데이션/radius/폰트/아바타ring/감정글로우색) · `d70ae15` 추천칩(가로스크롤→wrap pill+Sparkles) · `3213bb1`+`0f2b573` 한도UX(Alert→풀 업셀카드+직전메시지인용) · `7771b03` [중간](패딩/캡션색/카드폭/에러색) · `7680f09` 감정글로우

**AI 정보강화**: `9166e71` 네이버 게이트완화+Tavily심화+케어키워드 · `66119da` 앱패리티+Tavily광고블랙리스트 · `818e317` 위치없을때 동물병원 환각차단

**버그 fix**: `f355891` timeline minor4 · `e038927` 레벨 sparkle도트 제거(진짜범인) · `e7c90ec` quests 401 · `f90e427` telegram cron 2버그 · `28d6496` AI펫톡 crash(nearbyPlaces 옛형식) · `549400a` 사진첩스크롤/picker · `e0b9298` auth refresh token hang · `ab4c5a9` 홈카드 흔들림 · `5853243` 알림 본문모달(welcome) · `7069e01`+`0b530ef` /about + 로그인링크

**정정 사고**: `81762e7`→`6509917` 레벨 PNG 데코 제거가 오진(사용자: "원본엔 동물만, 점은 코드가 만든것") → revert 후 `e038927`로 진짜범인(LevelBadge sparkle) 수정.

**판명**: KCP MID 블로커 outdated(IP6S2 폴백+dojin3497 실결제 확인). 정기결제 자동갱신 시뮬레이션 L4 통과.

**미검증/외부대기**: RELAY.md 보드 ⚠️/⏳ 참조 (EAS 빌드·Vercel 배포 후 시각확인 다수).
