# 메멘토애니 인수인계 (2026-04-16 03:30 KST 업데이트)

다른 컴퓨터 + **새 Claude 세션**에서 이어갈 때 이 문서부터 보면 됨.

---

## 🧭 새 Claude 세션 시작 전 — 최신 상태 요약 (04-16 새벽 업데이트)

- 현재 시점: **2026-04-16 새벽 (금요일 직전)**
- 개발자: **안승빈** (sharkwind1@gmail.com)
- 서비스: **메멘토애니** (반려동물 메모리얼 커뮤니티, mementoani.com)
- 단계: **출시 직전** — 결제 연동만 외부 대기 중
- **결제 블로커**: KCP `[3014] 미등록 또는 사용불가 사이트 코드` — 코드/포트원 설정은 전부 정상, KCP 측 정기결제 모듈 활성화 대기. 상세는 `RELAY.md` 상단 섹션 참고.

**04-16 새벽 세션 요약 (외부 PC로 작업, PR 9개 머지)**:

| PR | 내용 |
|----|------|
| #15 | `/api/payments/complete` 에도 debug_* 진단 필드 + is_subscription 감지 구독 처리 추가 |
| #16 | 포트원 `request_pay`에 `pg: "kcp_billing.A52LD"` 명시 (KCP 3014 시도 — 해결 안 됨, 원인은 KCP 측) |
...
| #19 | 로그인 플로우 RPC 중복 3개 제거 + `auth.getUser()` → `getSession()` (로컬 캐시, 네트워크 제거) |
| #20 | `html2canvas` 동적 import — 초기 번들 ~75KB 감소 |
| #21 | `PetContext` pets + pet_media 병렬 쿼리 (순차 → 병렬) |
| #22 | `useHomePage` memorial 쿼리 병렬 + 20260412_admin_messages 마이그 실적용 (Supabase MCP) |
...
## 🚨 최우선 블로커: 결제 연동 (04-16 새벽 상태 업데이트)

> ⚠️ **에러 메시지가 바뀜**: "존재하지 않는 결제정보" → **"[3014] 미등록 또는 사용불가 사이트 코드"**
>
> 이전 에러(존재하지 않는 결제정보)는 04-15 Vercel 환경변수 교체 + V1 Secret 재발급으로 해결됨.
> 지금은 KCP 측에서 사이트 자체를 거절하는 3014 에러. **코드 측에서 더 시도할 건 없음.**
>
> 원인·해결 경로는 **`RELAY.md` 상단 "진행 중 블로커" 섹션**에 정리됨.
> 아래 이전 기록은 히스토리 보존용.
