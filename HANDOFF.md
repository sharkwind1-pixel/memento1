# 메멘토애니 인수인계 (2026-04-16 외부 PC 세션 종료 시점)

다른 컴퓨터 + **새 Claude 세션**에서 이어갈 때 이 문서부터 보면 됨.

---

## 🧭 새 Claude 세션 시작 전 — 최신 상태 요약 (04-16 종료)

- 현재 시점: **2026-04-16 (금) 오후 — 외부 PC 작업 종료, 집에서 이어갈 예정**
- 개발자: **안승빈** (sharkwind1@gmail.com)
- 서비스: **메멘토애니** (반려동물 메모리얼 커뮤니티, mementoani.com)
- 단계: **출시 직전 — 결제 외 모두 정상**

### ⚠️ 결제 블로커 (원인 확정 · 액션 대기)

KCP `[3014] 미등록 사이트 코드` 에러 원인 **포트원 Benny 매니저 답변으로 확정**:
> **A52LD는 포트원이 개발자에게 공유하는 공용 테스트 MID**.
> 실연동 채널에 이 값이 박혀있어서 KCP가 거절한 것.

**진짜 사이트코드 확인됨: `IP6S2`** (KCP 상점관리자 > 상점정보관리 > 사이트정보)
- 사이트명: 메멘토애니_자동결제
- 계약일시: 2026.04.02
- KCP 로그인 ID: mementoani2

**아직 못 받은 것 (KCP 영업/계약 담당자에게 받아야 함)**:
- 사이트키 (암호화키)
- 정기결제 그룹ID

→ 포트원 Benny 답변: "PG사에서 고객사로 전달. 계약 담당자 통해 확인 부탁."
→ **다음 액션: KCP 계약 담당자에게 메일/전화로 요청** (지메일에서 `from:kcp.co.kr` 검색해 담당자 찾기 또는 1544-8667 일반상담 연결).

받으면 포트원 콘솔 → 실연동 채널(`mementoani-real`) 수정 → A52LD/4wtvaFYQknd3bJLVxa03mHO__/A52LD1000913 → IP6S2 / 받은 사이트키 / 받은 정기그룹ID 로 교체 → 저장 후 결제 재시도.

### 04-16 세션 머지된 PR 13개

| PR | 내용 |
|----|------|
| #15 | `/api/payments/complete` 에도 debug_* 진단 필드 + is_subscription 감지 구독 처리 추가 |
| #16 | 포트원 `request_pay`에 `pg: "kcp_billing.A52LD"` 명시 (3014 해결 시도 — 원인은 MID 자체였음) |
| #17 | 네이버 OAuth 콜백 `listUsers` 풀스캔 제거 → profiles.email 인덱스 1회 조회 |
| #18 | AuthContext `handledUserIdRef`로 로그인 직후 refreshProfile 스킵 버그 수정 (**새로고침 없이 UI 뜨던 문제**) |
| #19 | 로그인 플로우 RPC 중복 3개 제거 + `auth.getUser()` → `getSession()` (로컬 캐시) |
| #20 | `html2canvas` 동적 import — 초기 번들 ~75KB 감소 |
| #21 | `PetContext` pets + pet_media 병렬 쿼리 |
| #22 | `useHomePage` memorial 쿼리 병렬 + `20260412_admin_messages` 마이그 실적용 |
| #23 | `.gitignore`에 `.claude/worktrees/` + PR 임시 파일 추가 |
| #24 | 04-16 새벽 세션 기록 (HANDOFF + RELAY 1차 업데이트) |
| #25 | Open 100 이벤트 기획 RELAY 기록 |
| #26 | **Open 100 오픈 이벤트 구현 (DB + API + 배너 + 토스트)** |
| #27 | Open 100 관리자 대시보드 섹션 + `/api/admin/open100` |

**예상 체감 개선 누적**: 로그인 ~1.5초↓, 초기 번들 ~75KB↓, 홈 진입 ~200ms↓.
**검증 레벨**: 전부 L0~L1 (외부 PC라 로컬 node 없음 → Vercel 빌드에 의존).

### Supabase 마이그 실적용 (Supabase MCP)
- `20260412_admin_messages` — `notifications.sender_id` + CHECK 확장 + 인덱스
- `20260416_open100_event` — `profiles.open100_awarded_at` + `award_open100(uuid)` RPC (advisory lock)

### Open 100 오픈 이벤트 (구현 완료, 배포 대기)
- 출시 기념 선착순 100명: 온보딩 미션 전체 완주 시 1,000P 자동 지급
- 대상: `OPEN100_EVENT_START_AT` (`2026-04-16T03:00+09:00`) 이후 가입자 + TRUSTED_EMAILS 제외
- DB는 RPC + advisory lock으로 동시 101명 지급 방지
- UI: 홈/간편홈 상단 `Open100Banner` (남은 자리 N명 프로그레스 바)
- 알림: 텔레그램 시스템 그룹으로 매 달성마다 (+ 100명 채워지면 축하 메시지)
- 관리자: 대시보드 탭 하단 `Open100AdminSection` (지급자 목록 100건)

### 첫 응답 시 주의사항 (이전 HANDOFF에서 유지)
...
## 🚨 최우선 블로커: 결제 연동 (04-16 오후 — 원인 확정)

> ⚠️ **에러 변천사**:
> 1. (04-15) "존재하지 않는 결제정보" → Vercel 환경변수 교체 + V1 Secret 재발급으로 해결
> 2. (04-16 새벽) "[3014] 미등록 또는 사용불가 사이트 코드" → 원인 미상으로 KCP/포트원 문의
> 3. (04-16 오후) 포트원 Benny 매니저 답변: **A52LD는 포트원 공용 테스트 MID**. 실연동 채널에 잘못 들어가 있어 KCP가 거절. 진짜 사이트코드는 **`IP6S2`** (KCP 상점관리자에서 확인됨).
>
> **남은 액션**: KCP 영업/계약 담당자에게 사이트키(암호화키) + 정기결제 그룹ID 요청 → 받으면 포트원 콘솔 실연동 채널의 A52LD/사이트키/정기그룹ID를 모두 교체. 자세한 가이드는 `RELAY.md` 상단 "결제 블로커" 섹션.
>
> 아래 이전 기록은 히스토리 보존용.

### (히스토리) 이전 증상 — 존재하지 않는 결제정보
