...
### 마이그레이션 규칙 (2026-02-22 사후분석에서 도출)
- SQL 파일 작성 = 작업의 50%. 실행 + 검증까지 해야 100%
- `ADD COLUMN IF NOT EXISTS`는 기존 컬럼 타입이 달라도 무시하므로, 타입 변경이 필요하면 `ALTER COLUMN TYPE`을 반드시 포함
- 마이그레이션 실행: `npx tsx scripts/run-migration.ts <파일명>` (.env.local 자동 로딩)
- 실행 불가 시: RELAY.md 최상단 "미실행 마이그레이션" 테이블에 기재 (파일명, SQL, 영향 기능, 긴급도)
