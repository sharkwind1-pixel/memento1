 ...
 *
 * 스크롤 전략:
 * - useBodyScrollLock 사용 안 함 (position:fixed가 모든 스크롤을 죽임)
 * - backdrop: fixed inset-0, flex items-center justify-center (화면 정중앙)
 * - 모달 본체: max-h-[85dvh] overflow-y-auto (모달 내부에서 스크롤)
 * - 배경 터치 시 모달 닫기
 */
...
export default function MemorialSwitchModal({
    ...
    // useBodyScrollLock 대신: 배경 터치무브만 막기
    useEffect(() => {
        if (!isOpen) return;

        const handler = (e: TouchEvent) => {
            // 모달 내부 터치는 허용, 배경 터치만 차단
    ...
    return (
        /* backdrop: 화면 정중앙에 모달 배치 */
        <div
            className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4"
            onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}
        >
            {/* 모달 본체: 최대 높이 85dvh, 내부 스크롤 */}
            <div
                ref={modalRef}
                className="bg-white dark:bg-gray-900 rounded-3xl max-w-md w-full shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 max-h-[85dvh] overflow-y-auto overscroll-contain"
