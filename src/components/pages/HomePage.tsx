...
// HERO 메시지 (모든 유저 공통 - 깜빡임 방지를 위해 통일)
const HERO_CONTENT = {
    title: "특별한 매일을 함께",
                        ...
                        {communityPosts.map((post, idx) => {
                            ...
                            return (
                                <Card
                                    ...
                                    className="w-[260px] sm:w-72 flex-shrink-0 overflow-hidden rounded-2xl cursor-pointer group border-0 shadow-lg will-change-transform"
                                >
                                    {/* 상단 그라데이션 배너 */}
                        ...
                        >
                            {bestPosts.adoption.map((pet, i) => {
                                ...
                                return (
                                    <Card
                                        key={i}
                                        className="w-[260px] sm:w-72 flex-shrink-0 bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden shadow-sm will-change-transform"
                                    >
                ...
                <section className="space-y-6 px-4">
                    ...
                    >
                        {bestPosts.petcare.map((guide, i) => (
                            <Card
                                key={i}
                                className="w-[260px] sm:w-64 flex-shrink-0 bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm will-change-transform"
                            >
                                <CardHeader>
                    ...
                    >
                        {isLoadingMemorial ? (
                            ...
                            Array.from({ length: 3 }).map((_, i) => (
                                <Card
                                    key={`skeleton-${i}`}
                                    className="w-[260px] sm:w-72 flex-shrink-0 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 border-amber-100 dark:border-amber-800/50 rounded-2xl overflow-hidden shadow-sm animate-pulse"
                                >
                            ...
                            displayMemorialData.map((m, i) => {
                                ...
                                return (
                                    <Card
                                        key={m.id}
                                        className="w-[260px] sm:w-72 flex-shrink-0 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 border-amber-100 dark:border-amber-800/50 rounded-2xl overflow-hidden shadow-sm will-change-transform"
                                    >
                                        <CardHeader className="p-0 relative">
                                            {src ? (
                                                <button
