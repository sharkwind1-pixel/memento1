...
import { ADMIN_EMAILS, TESTER_EMAILS } from "@/config/constants";
...
export default function PetProfileCard({
    ...
    // 일상 모드 복구는 관리자 또는 화이트리스트 테스터 계정만 (실수 복구 방지)
    const email = (user?.email ?? "").toLowerCase();
    const canRecoverToActive = isAdminUser
        || ADMIN_EMAILS.some((e) => e.toLowerCase() === email)
        || TESTER_EMAILS.some((e) => e.toLowerCase() === email);
    return (
