...
export interface VisitStats {
    today: {
        totalVisits: number;     // 오늘 전체 방문(세션) 수
        uniqueVisitors: number;  // 오늘 고유 방문자(visitor_id 기준)
        guestVisitors: number;   // 그중 비로그인(게스트)
        memberVisitors: number;  // 그중 로그인 회원
    };
    daily: Array<{
        date: string;     // M/D
        방문자: number;   // 고유 방문자(게스트+회원)
        비로그인: number; // 게스트 고유 방문자
    }>;
    /** 가입 전환 퍼널 (최근 7일, 단계별 고유 방문자 → drop-off) */
    funnel?: Array<{
        step: string;     // 방문 / 둘러봄 / 가입 클릭 / 가입 완료
        visitors: number;
    }>;
}
