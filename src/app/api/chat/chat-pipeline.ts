...
export async function buildAIContext(
    ...
    if (serverDetection.detected) {
        ...
        // 장소 질문 감지됐으나 실제 검색 결과 0건 (위치정보 없음 / 검색 실패)
        // → GPT가 학습데이터로 실재하지 않는 병원/장소 이름을 환각하는 것 강제 차단
        //   (실사례: "근처 동물병원" → "산타로사/힐즈버그 캠퍼스" 미국 지명 환각)
        if (nearbyPlaces.length === 0) {
            placeContext = `\n[최우선 — 장소 질문 감지됐으나 위치 정보 없음 / 검색 결과 0건]
사용자가 장소(동물병원/산책로/펫카페 등)를 물었지만, 실제 위치 검색을 하지 못했습니다.
절대 금지: 구체적인 가게·병원·공원·시설의 "이름"을 지어내지 마세요.
  (실재하지 않는 곳·외국 지명·캠퍼스명 등을 말하면 사용자가 실제로 찾아가려다 큰 피해)
반드시 다음 순서로 응답:
1. 위치를 몰라서 정확히는 모른다고 솔직하게 (펫 1인칭 톤, 자연스럽게)
2. "동네 이름 알려주면 찾아볼게!" 또는 "위치 공유해주면 근처 알려줄 수 있어!" 식으로 요청
3. 이름 없는 일반 팁만 (예: 가기 전 전화로 진료시간 확인, 24시 병원 번호 미리 저장)
예시 톤: "음, 네가 어디 사는지 몰라서 정확한 병원 이름은 모르겠어ㅠ 동네나 지역 이름 알려주면 근처로 찾아볼게! 그동안엔 가까운 24시 동물병원 번호 하나 저장해두면 든든해~"`;
        }
    }
...
export async function postProcessResponse(
    ...
    if (isMemorialMode) {
        ...
        // ============================================================
        // 추모 모드 보호자 시점 자동 정정 (Phase 1: prompt만으로 못 막는 패턴 후처리)
        // 사용자 짜증 1순위: "너의 웃는 모습", "모습이 선명해" 등 보호자 외부 시점.
        // 정규식으로 명백한 케이스만 치환. 미묘한 케이스는 prompt에 의존.
        // ============================================================
        const beforeRewrite = reply;
        reply = reply
            // "너의 OO는 모습" / "너의 OO하는 모습" → "네가 OO하던 거" (보호자 시점 → 동물 시점)
            .replace(/너의\s+([가-힣]{1,4})는\s+모습이?\s*/g, "네가 $1던 거 ")
            .replace(/너의\s+([가-힣]{1,4})하는\s+모습이?\s*/g, "네가 $1하던 거 ")
            .replace(/너의\s+([가-힣]{1,4})한\s+모습이?\s*/g, "네가 $1했던 거 ")
            // 보호자 외부 시점 단어 → 내부 회상으로
            .replace(/(?:그 )?모습이\s*선명해/g, "그 순간이 잊히지 않아")
            .replace(/(?:그 )?모습이?\s*떠올라/g, "그 순간이 생각나")
            .replace(/(?:그 )?장면이?\s*떠올라/g, "그 순간이 생각나")
            .replace(/(?:그 )?모습이?\s*보여/g, "그 순간이 생각나")
            // 동물이 사진 회상 어색 → 시간으로
            .replace(/같이\s*찍은\s*사진들?/g, "함께한 순간들")
            // 추모에서 발랄 시작 표현 절제
            .replace(/^\s*물론이지[!.~]*\s*/, "... 응. ")
            .replace(/^\s*당연하지[!.~]*\s*/, "... 그래. ")
            // 추모에서 "기분이 좋아져" 같은 가벼운 표현 → 잔잔하게
            .replace(/기분이\s*(?:정말\s*|너무\s*|진짜\s*)?좋아져/g, "마음이 차분해져")
            .replace(/신난다[!.~]*/g, "좋아.")
            .replace(/너무\s*신나/g, "마음이 따뜻해");

        if (beforeRewrite !== reply) {
            console.warn(`[chat/memorial-tone-rewrite] 보호자 시점/발랄 표현 자동 정정됨. pet=${pet.name}`);
        }
    }
