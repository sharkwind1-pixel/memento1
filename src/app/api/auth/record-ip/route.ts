        ...
        if (deviceFingerprint && !isCurrentUserTrusted) {
            try {
                ...
                const untrustedSameDevice = (sameDevice || []).filter(
                    (p: { email: string | null }) => !p.email || !TRUSTED_EMAILS.includes(p.email)
                );
                ...
                if (untrustedSameDevice.length >= 2) {
                    ...
                    import("@/lib/telegram").then(({ notifyError }) =>
                        notifyError({
                            ...
                            error: `같은 디바이스에서 ${untrustedSameDevice.length + 1}개 계정 감지. user=${user.id}, email=${userEmail}, 기존계정=${untrustedSameDevice.map((p: { email: string | null }) => p.email).join(",")}`,
                        })
                    ).catch(() => {});
                }
            } catch {
