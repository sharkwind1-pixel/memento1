 ...
 * - 눈: 기본 BK 1px 점. 품종 특성이 강한 경우 컬러눈(2px accent + BK) 허용
 *   (러시안블루: 에메랄드, 스코티시폴드: 앰버, 게코: 오렌지)
...
const kshGrid: (string | null)[][] = [
    ...
    [_, _, _, _, cC, cC, _, _, _, cC, cC, _, _, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, cC, cB, cC, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, _, cC, cB, _],
];
...
const russianBlueGrid: (string | null)[][] = [
    [_, _, bC, _, _, _, _, _, _, _, _, _, _, bC, _, _],
    [_, bC, bA, bC, _, _, _, _, _, _, _, bC, bA, bC, _, _],
    [_, bC, bA, bA, bC, bC, bC, bC, bC, bC, bC, bA, bA, bC, _, _],
    [_, _, bC, bA, bA, bA, bA, bA, bA, bA, bA, bA, bC, _, _, _],
    [_, _, bC, bE, bE, BK, bA, bA, bA, bE, bE, BK, bC, _, _, _],
    [_, _, bC, bA, bA, bA, bA, bA, bA, bA, bA, bA, bC, _, _, _],
    [_, _, _, bC, bA, bA, bA, BK, bA, bA, bA, bC, _, _, _, _],
    [_, _, _, bC, bA, bA, PK, bW, PK, bA, bA, bC, _, _, _, _],
    [_, _, _, _, bC, bC, bC, bC, bC, bC, bC, _, _, _, _, _],
    [_, _, _, _, _, bC, bA, bW, bA, bC, _, _, _, _, _, _],
    [_, _, _, _, _, bC, bW, bW, bW, bC, _, _, _, _, _, _],
    [_, _, _, _, _, bC, bA, bA, bA, bC, _, _, _, _, _, _],
    [_, _, _, _, _, bC, bC, _, bC, bC, _, _, _, _, _, _],
    [_, _, _, _, _, bC, bC, _, bC, bC, _, _, _, _, _, _],
...
const fA = "#e8d0a0"; // cream beige base
const fB = "#c8a870"; // warm beige shadow
const fC = "#987848"; // brown outline
const fW = "#f5e8d0"; // belly light cream
const fE = "#e8a020"; // amber eye

const scottishFoldGrid: (string | null)[][] = [
    [_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _],
    [_, _, _, fB, fC, fC, fC, fC, fC, fC, fC, fC, fB, _, _, _],
    [_, _, fC, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fC, _, _],
    [_, fC, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fC, _],
    [_, fC, fA, fA, fE, BK, fA, fA, fA, fA, fE, BK, fA, fA, fC, _],
    [_, fC, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fA, fC, _],
    [_, fC, fA, fA, fA, fA, fA, BK, fA, fA, fA, fA, fA, fA, fC, _],
    [_, _, fC, fA, fA, fC, PK, fW, PK, fC, fA, fA, fC, _, _, _],
    [_, _, _, fC, fC, fC, fC, fC, fC, fC, fC, fC, _, _, _, _],
    [_, _, fC, fA, fA, fW, fW, fW, fW, fW, fA, fA, fC, _, _, _],
    [_, fC, fA, fA, fW, fW, fW, fW, fW, fW, fW, fA, fA, fC, _, _],
    [_, fC, fA, fA, fA, fA, fW, fW, fW, fA, fA, fA, fA, fC, _, _],
    [_, _, fC, fC, _, _, _, _, _, _, _, fC, fC, _, _, _],
    [_, _, fC, fC, _, _, _, _, _, _, _, fC, fC, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, fC, fB, fC, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, fC, _, _, _],
];
...
const pA = "#64d4c0"; // teal light (게코와 구분되는 청록)
const pB = "#3db8a8"; // teal mid
const pC = "#1a8a78"; // teal dark
...
const parrotGrid: (string | null)[][] = [
    ...
    [_, _, _, _, pC, pA, pA, pA, pA, pA, pC, _, _, _, _, _],
    [_, _, _, _, _, pC, pA, pO, pA, pC, _, _, _, _, _, _],
    [_, _, _, _, _, _, pC, pO, pC, _, _, _, _, _, _, _],
    ...
    [_, _, _, _, _, _, pC, pB, pC, _, _, _, _, _, _, _],
];
...
const geckoGrid: (string | null)[][] = [
    [_, _, _, kE, kE, _, _, _, _, _, _, kE, kE, _, _, _],
    [_, _, kE, BK, kE, kC, _, _, _, _, kC, kE, BK, kE, _, _],
    [_, _, _, kC, kA, kA, kA, kA, kA, kA, kA, kA, kC, _, _, _],
    [_, _, _, kC, kA, kA, kA, kA, kA, kA, kA, kA, kC, _, _, _],
    [_, _, _, _, kC, kA, kA, kA, kA, kA, kA, kC, _, _, _, _],
    [_, _, _, _, kC, kA, PK, BK, PK, kA, kA, kC, _, _, _, _],
    [_, _, _, _, _, kC, kA, kA, kA, kA, kC, _, _, _, _, _],
    [_, _, _, _, _, kC, kA, kA, kA, kA, kC, _, _, _, _, _],
    [_, _, _, _, kC, kA, kA, kA, kA, kA, kC, _, _, _, _, _],
    [_, kB, kC, _, kC, kA, kW, kW, kW, kA, kC, _, kC, kB, _, _],
    [_, _, _, kC, kA, kA, kW, kW, kA, kA, kA, kC, _, _, _, _],
    [_, _, _, _, kC, kA, kA, kA, kA, kA, kC, _, _, _, _, _],
    [_, _, _, _, kC, kC, _, _, _, kC, kC, _, _, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, kC, kB, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, kC, kB, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, _, kC, kB, _],
];
...
const hamsterGrid: (string | null)[][] = [
    ...
    [_, _, _, _, hC, hC, _, _, _, hC, hC, _, _, _, _, _],
    [_, _, _, _, hA, hA, _, _, _, hA, hA, _, hB, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _],
    [_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _],
];
