...
- spam: 명백한 상업 스팸/광고만 (제3자 상품·서비스 판매/홍보, 외부 쇼핑·도박·대출·성인 링크, 연락처·오픈채팅·DM 유도, 할인코드/프로모션, 동일 내용 반복 도배)
...
- 사용자가 직접 다녀온 맛집·장소·병원·제품 후기나 추천, 일상 공유는 광고가 아니라 정상(safe)입니다. 상호명·지명이 들어가도 본인 경험을 나누는 글이면 safe.
- 판매·홍보 의도(외부 링크/연락처/할인/반복 도배)가 분명할 때만 spam으로 판정하고, 애매하면 반드시 safe로 판정하세요.
...
export async function moderateWithAI(
    ...
    try {
        ...
        // 판정 결과에 따른 처리.
        // 자동 숨김은 명백한 위반(선정/폭력/혐오)만. spam은 오탐(맛집·제품 후기→광고 오인)이 잦아
        // 숨기지 않고 "검토 필요" 플래그만 → 관리자가 텔레그램 /hide로 수동 처리. (오탐으로 정상글 사라짐 방지)
        const HIDE_VERDICTS: ModerationVerdict[] = ["inappropriate", "hate_speech"];
        const shouldHide = HIDE_VERDICTS.includes(verdict);
        const moderationStatus = verdict === "safe" ? "approved" : shouldHide ? "rejected" : "flagged";

        // community_posts 업데이트 (숨김은 shouldHide일 때만)
        await supabase
            .from("community_posts")
            .update({
                moderation_status: moderationStatus,
                moderation_reason: verdict !== "safe" ? reason : null,
                ...(shouldHide && { is_hidden: true }),
            })
            .eq("id", postId);
        ...
        await supabase.from("moderation_logs").insert({
            post_id: postId,
            filter_type: "ai",
            result: verdict === "safe" ? "approved" : shouldHide ? "blocked" : "flagged",
        ...
        if (verdict !== "safe") {
            // 텔레그램 신고 그룹 알림 — 숨김 여부 명시(spam은 숨김 안 하고 검토만)
            const action = shouldHide ? "자동 숨김" : "검토 필요(숨김 안 함)";
            import("@/lib/telegram").then(({ notifyReport }) =>
                notifyReport({
                    targetType: "ai-moderation",
                    targetId: postId,
                    reason: `AI ${action}: ${verdict} - ${reason || "부적절한 콘텐츠"}`,
                })
            ).catch(() => {});
        }
    } catch (err) {
